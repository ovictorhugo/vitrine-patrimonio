...
const first = (v: string | null) => (v ? v.split(";").filter(Boolean)[0] ?? "" : "" as string);
  ...
  const navigate = useNavigate();
   ...
   /** ===== Helpers URL ===== */
    ...
    const fetchMaterials = useCallback(async () => {
      try {
        const res = await fetch(`${baseUrl}/materials/`, { headers: baseHeaders });
        const json = await res.json();
        const list: Material[] = Array.isArray(json?.materials) ? json.materials : Array.isArray(json) ? json : [];
        setMaterials(list);
      } catch {}
    }, [baseUrl, baseHeaders]);
    ...
    const fetchGuardians = useCallback(async () => {
      try {
        const res = await fetch(`${baseUrl}/legal-guardians/`, { headers: baseHeaders });
        const json = await res.json();
        const list: LegalGuardian[] = Array.isArray(json?.legal_guardians) ? json.legal_guardians : Array.isArray(json) ? json : [];
        setGuardians(list);
      } catch {}
    }, [baseUrl, baseHeaders]);
    ...
    const fetchLocations = useCallback(async () => {
      try {
        const res = await fetch(`${baseUrl}/locations/`, { headers: baseHeaders });
        const json = await res.json();
        const list: LocationT[] = Array.isArray(json?.locations) ? json.locations : Array.isArray(json) ? json : [];
        setLocations(list);
      } catch {}
    }, [baseUrl, baseHeaders]);
    ...
    const fetchUnits = useCallback(async () => {
      try {
        ...
        const json = await res.json();
        const list: Unit[] = Array.isArray(json?.units) ? json.units : Array.isArray(json) ? json : [];
        setUnits(list);
      } catch {}
    }, [baseUrl, baseHeaders]);
    ...
    const fetchAgencies = useCallback(async () => {
      try {
        ...
        const json = await res.json();
        const list: Agency[] = Array.isArray(json?.agencies) ? json.agencies : Array.isArray(json) ? json : [];
        setAgencies(list);
      } catch {}
    }, [baseUrl, baseHeaders]);
    ...
    const fetchSectors = useCallback(async () => {
      try {
        ...
        const json = await res.json();
        const list: Sector[] = Array.isArray(json?.sectors) ? json.sectors : Array.isArray(json) ? json : [];
        setSectors(list);
      } catch {}
    }, [baseUrl, baseHeaders]);
    ...
    useEffect(() => {
      ...
      fetchAgencies();
      fetchSectors();
    }, [fetchMaterials, fetchGuardians, fetchLocations, fetchUnits, fetchAgencies, fetchSectors]);
    ...
    /** ===== Nomes para chips ===== */
    const materialName = useMemo(() => materials.find((m) => m.id === selectedMaterial)?.material_name || "", [materials, selectedMaterial]);
    const guardianName = useMemo(() => guardians.find((g) => g.id === selectedLegalGuardian)?.legal_guardians_name || "", [guardians, selectedLegalGuardian]);
    const locationName = useMemo(() => locations.find((l) => l.id === selectedLocation)?.location_name || "", [locations, selectedLocation]);
    const unitName = useMemo(() => units.find((u) => u.id === selectedUnit)?.unit_name || "", [units, selectedUnit]);
    const agencyName = useMemo(() => agencies.find((a) => a.id === selectedAgency)?.agency_name || "", [agencies, selectedAgency]);
    const sectorName = useMemo(() => sectors.find((s) => s.id === selectedSector)?.sector_name || "", [sectors, selectedSector]);
    ...
    const clearOne = (key: "material" | "guardian" | "location" | "unit" | "agency" | "sector") => {
      const sp = new URLSearchParams(location.search);
      if (key === "material") { setSelectedMaterial(""); sp.delete("material_ids"); }
      if (key === "guardian") { setSelectedLegalGuardian(""); sp.delete("legal_guardian_ids"); }
      if (key === "location") { setSelectedLocation(""); sp.delete("location_ids"); }
      if (key === "unit") { setSelectedUnit(""); sp.delete("unit_ids"); }
      if (key === "agency") { setSelectedAgency(""); sp.delete("agency_ids"); }
      if (key === "sector") { setSelectedSector(""); sp.delete("sector_ids"); }
      sp.set("offset", "0");
      setOffset(0);
      navigate({ pathname: location.pathname, search: sp.toString() });
    };
...
<Alert className="h-14 mt-4 mb-2 p-2 flex items-center justify-between">
                  <div className="flex items-center gap-2 w-full flex-1">
                    ...
                    <div className="hidden md:flex gap-2 w-fit items-center">
                     ...
                     {(selectedMaterial || selectedLegalGuardian || selectedLocation || selectedUnit || selectedAgency || selectedSector) && (
  <div className="flex gap-2 mx-2 items-center">
    {[
      { kind: "material", value: selectedMaterial, label: materialName },
      { kind: "guardian", value: selectedLegalGuardian, label: guardianName },
      { kind: "location", value: selectedLocation, label: locationName },
      ...
      ({ kind, value, label }) =>
        value && (
          <div
            key={kind}
            className={`flex gap-2 items-center h-10 p-2 px-4 capitalize rounded-md text-xs text-white ${KIND_BG[kind as Kind]}`}
          >
            {label || value}
            <X size={12} onClick={() => clearOne(kind as Kind)} className="cursor-pointer" />
          </div>
        )
    )}
  </div>
                  ...
                  <div className="w-fit flex gap-2">
                    {(selectedMaterial || selectedLegalGuardian || selectedLocation || selectedUnit || selectedAgency || selectedSector) && (
                      <Button size={'icon'} variant={'ghost'} onClick={() => clearAllFilters()}>
                        <Trash size={16} />
                      </Button>
                    )}
  ...
  variant="outline"
  className={`${BTN_COLOR(activeKind)} text-white hover:text-white  border-0`}
  size="icon"
>
