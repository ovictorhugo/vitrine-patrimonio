...
export const KIND_BG: Record<Kind, string> = {
  material: "bg-blue-600 hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-700",
  guardian: "bg-rose-600 hover:bg-rose-700 dark:bg-rose-600 dark:hover:bg-rose-700",
  location: "bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-600 dark:hover:bg-emerald-700",
  unit: "bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-700",
  agency: "bg-teal-600 hover:bg-teal-700 dark:bg-teal-600 dark:hover:bg-teal-700",
  sector: "bg-amber-600 hover:bg-amber-700 dark:bg-amber-600 dark:hover:bg-amber-700",
};
...
export function SearchModal() {
    ...
    (async () => {
      try {
        const [mres, gres, lres, ures, ares, sres] = await Promise.all([
          fetch(`${baseUrl}/materials/`, { headers: baseHeaders }).then((r) => r.json()),
          fetch(`${baseUrl}/legal-guardians/`, { headers: baseHeaders }).then((r) => r.json()),
          fetch(`${baseUrl}/locations/`, { headers: baseHeaders }).then((r) => r.json()),
          fetch(`${baseUrl}/units/`, { headers: baseHeaders }).then((r) => r.json()),
          fetch(`${baseUrl}/agencies/`, { headers: baseHeaders }).then((r) => r.json()),
          fetch(`${baseUrl}/sectors/`, { headers: baseHeaders }).then((r) => r.json()),
        ]);

        const _materials: Material[] = Array.isArray(mres?.materials) ? mres.materials : Array.isArray(mres) ? mres : [];
        const _guards: LegalGuardian[] = Array.isArray(gres?.legal_guardians) ? gres.legal_guardians : Array.isArray(gres) ? gres : [];
        const _locs: LocationT[] = Array.isArray(lres?.locations) ? lres.locations : Array.isArray(lres) ? lres : [];
        const _units: Unit[] = Array.isArray(ures?.units) ? ures.units : Array.isArray(ures) ? ures : [];
        const _agencies: Agency[] = Array.isArray(ares?.agencies) ? ares.agencies : Array.isArray(ares) ? ares : [];
        const _sectors: Sector[] = Array.isArray(sres?.sectors) ? sres.sectors : Array.isArray(sres) ? sres : [];
        ...
        if (fromUrl) {
          ...
          if (m) return setPicked({ kind: "material", id: m.id, label: m.material_name });
          const g = _guards.find((x) => x.id === fromUrl);
          if (g) return setPicked({ kind: "guardian", id: g.id, label: g.legal_guardians_name });
          const l = _locs.find((x) => x.id === fromUrl);
          if (l) return setPicked({ kind: "location", id: l.id, label: l.location_name });
          const u = _units.find((x) => x.id === fromUrl);
          if (u) return setPicked({ kind: "unit", id: u.id, label: u.unit_name });
          const a = _agencies.find((x) => x.id === fromUrl);
          if (a) return setPicked({ kind: "agency", id: a.id, label: a.agency_name });
          const s = _sectors.find((x) => x.id === fromUrl);
          if (s) return setPicked({ kind: "sector", id: s.id, label: s.sector_name });
        }
      } catch (e) {
        console.error(e);
      }
  ...
  useEffect(() => {
    ...
    const first = (key: string) => (sp.get(key) || "").split(";").filter(Boolean)[0];
    ...
    if (trySet(mid, materials, (m: any) => ({ id: m.id, label: m.material_name }), "material")) return;
    if (trySet(gid, guardians, (g: any) => ({ id: g.id, label: g.legal_guardians_name }), "guardian")) return;
    if (trySet(lid, locations, (l: any) => ({ id: l.id, label: l.location_name }), "location")) return;
    if (trySet(uid, units, (u: any) => ({ id: u.id, label: u.unit_name }), "unit")) return;
    if (trySet(aid, agencies, (a: any) => ({ id: a.id, label: a.agency_name }), "agency")) return;
    if (trySet(sid, sectors, (s: any) => ({ id: s.id, label: s.sector_name }), "sector")) return;

    // se nada na URL, zera
    setPicked(null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [location.search, materials, guardians, locations, units, agencies, sectors]);
  ...
  const matsView = useMemo(
    () => (showSuggestions ? materials.filter((m) => norm(m.material_name).includes(term) || norm(m.material_code).includes(term)).slice(0, 30) : []),
    [materials, term, showSuggestions]
  );
  const guardsView = useMemo(
    () => (showSuggestions ? guardians.filter((g) => norm(g.legal_guardians_name).includes(term) || norm(g.legal_guardians_code).includes(term)).slice(0, 30) : []),
    [guardians, term, showSuggestions]
  );
  const locsView = useMemo(
    () => (showSuggestions ? locations.filter((l) => norm(l.location_name).includes(term) || norm(l.location_code).includes(term)).slice(0, 30) : []),
    [locations, term, showSuggestions]
  );
  const unitsView = useMemo(
    () => (showSuggestions ? units.filter((u) => norm(u.unit_name).includes(term) || norm(u.unit_code).includes(term)).slice(0, 30) : []),
    [units, term, showSuggestions]
  );
  const agenciesView = useMemo(
    () => (showSuggestions ? agencies.filter((a) => norm(a.agency_name).includes(term) || norm(a.agency_code).includes(term)).slice(0, 30) : []),
    [agencies, term, showSuggestions]
  );
  const sectorsView = useMemo(
    () => (showSuggestions ? sectors.filter((s) => norm(s.sector_name).includes(term) || norm(s.sector_code).includes(term)).slice(0, 30) : []),
  ...
  const apply = () => {
    ...
    if (picked) {
      const param =
        picked.kind === "material" ? "material_ids" :
        picked.kind === "guardian" ? "legal_guardian_ids" :
        picked.kind === "location" ? "location_ids" :
        picked.kind === "unit" ? "unit_ids" :
        picked.kind === "agency" ? "agency_ids" :
        "sector_ids";
          ...
          <Alert className="w-full border-t-0">
            <ResponsiveMasonry columnsCountBreakPoints={{ 350: 1, 750: 2, 900: 2, 1200: 3 }}>
              <Masonry className="max-h-[70vh] md:overflow-y-auto overflow-y-scroll" gutter="20px">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      {matsView.map((m) => (
                        <div
                          key={m.id}
                          onClick={() => choose("material", m.id, m.material_name)}
                ...
                {guardsView.length > 0 && (
                  <div>
                    <p className="uppercase font-medium text-xs mb-3">Responsáveis</p>
                    <div className="flex flex-wrap gap-3">
                      {guardsView.map((g) => (
                        <div
                          key={g.id}
                          onClick={() => choose("guardian", g.id, g.legal_guardians_name)}
                          className="flex gap-2 h-8 capitalize cursor-pointer transition-all bg-neutral-100 hover:bg-neutral-200 dark:hover:bg-neutral-900 dark:bg-neutral-800 items-center p-2 px-3 rounded-md text-xs"
                        >
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      {locsView.map((l) => (
                        <div
                          key={l.id}
                          onClick={() => choose("location", l.id, l.location_name)}
                          className="flex gap-2 h-8 capitalize cursor-pointer transition-all bg-neutral-100 hover:bg-neutral-200 dark:hover:bg-neutral-900 dark:bg-neutral-800 items-center p-2 px-3 rounded-md text-xs"
                        >
                          {l.location_name}
                        </div>
                ...
                {unitsView.length > 0 && (
                  <div>
                    <p className="uppercase font-medium text-xs mb-3">Unidades</p>
                    <div className="flex flex-wrap gap-3">
                      {unitsView.map((u) => (
                        <div
                          key={u.id}
                ...
                {agenciesView.length > 0 && (
                  <div>
                    <p className="uppercase font-medium text-xs mb-3">Organizações</p>
                    <div className="flex flex-wrap gap-3">
                      {agenciesView.map((a) => (
                        <div
                          key={a.id}
                ...
                {sectorsView.length > 0 && (
                  <div>
                    <p className="uppercase font-medium text-xs mb-3">Setores</p>
                    <div className="flex flex-wrap gap-3">
                      {sectorsView.map((s) => (
                        <div
                          key={s.id}
