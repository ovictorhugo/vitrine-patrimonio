...
const isCod = (i: SearchItem): i is Extract<SearchItem, { kind: "cod" }> =>
  i.kind === "cod";
...
export function SearchModalVitrine() {
  ...
  async function searchAssetIdentifier(q: string): Promise<SearchItem[]> {
    ...
    const ids = Array.isArray(data?.asset_identifier)
    ...
    return ids.map((id) => {
      ...
      return {
        kind: "cod",
        bem_cod: String(bem_cod),
        bem_dgv: String(bem_dgv),
      };
    });
  }
  ...
  async function searchGuardians(q: string): Promise<SearchItem[]> {
    ...
    const data = await fetchJson<{
      legal_guardians?: Array<{
        id: string;
        legal_guardians_code: string;
        legal_guardians_name: string;
      }>;
    }>(fullUrl);
    const arr = Array.isArray(data?.legal_guardians)
      ? data!.legal_guardians
      : [];
    return arr.map((g) => ({
      kind: "guardian",
      id: String(g.id),
  ...
  const choose = (k: Kind, item: SearchItem) => {
    if (k === "cod" && isCod(item)) {
      setPicked({
        kind: "cod",
        label: `${item.bem_cod}-${item.bem_dgv}`,
        payload: { bem_cod: item.bem_cod, bem_dgv: item.bem_dgv },
      });
    } else if (k === "atm" && isAtm(item)) {
      setPicked({
        kind: "atm",
        label: item.bem_num_atm,
        payload: { bem_num_atm: item.bem_num_atm },
      });
      ...
      setPicked({
        kind: "guardian",
        id: item.id,
        label: `${item.legal_guardians_code} • ${item.legal_guardians_name}`,
      });
    }
    setInput("");
  };
