...
export function AtualizarCadastro() {
    ...
    const isModalOpen = isOpen && type === 'atualizar-cadastro'
    return(
        <>
        {isModalOpen && (
             <div className="h-screen w-full flex items-center mx-16 relative">
                ...
                <h2 className=" text-3xl font-bold mb-1 max-w-[450px]">Faça login ou continue como anônimo para prosseguir.</h2>
                    ...
                    <Link to={'/signIn'}><Button ><SignIn size={16} className="" />Fazer login</Button></Link>
                        <Link to={'/signUp'}><Button ><UserPlus size={16} className="" />Criar conta</Button></Link>

                        ou

                        <Button variant={'outline'}><Detective size={16} className="" />Continuar como anônimo</Button>
                    </div>
                </div>
             </div>
        )}
