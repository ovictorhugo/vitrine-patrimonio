...
import {
  FormularioStep as FormularioStepView,
  Patrimonio,
} from "../dashboard/novo-item/steps/formulario";
...
export function BuscaCatalogo() {
  ...
  async function goToAsset() {
    ...
    const searchItem = wizard.pesquisa?.value_item;

    const found = assets.catalogs.find(
      (c) => c.asset_identifier === searchItem
    );

    if (found) {
      navigate("/item?id=".concat(found.catalog_id));
    }
  }
  ...
  const resetAll = useCallback(() => {
    ...
    setValid(() => {
      const next: ValidMap = {};
      for (const s of STEPS)
        (next as any)[s.key] = s.key === "pesquisa" ? false : undefined;
      return next;
    });
  ...
  const currentKey = `${wizard.pesquisa?.type ?? ""}::${
    wizard.pesquisa?.value_item ?? ""
  }`;
  ...
  const mapFormularioToAsset = (form?: Patrimonio): Asset | undefined => {
    ...
    const unit: any = {
      ...
      unit_name:
        f.unit?.unit_name ?? f.unit?.name ?? agency.unit?.unit_name ?? "",
  ...
  const openSavedItem = useCallback(
    (s: SavedItem) => {
      setWizard({
        pesquisa: { ...s.pesquisa },
        formulario: (s.asset as unknown as Patrimonio) ?? wizard.formulario,
      });
  ...
  const downloadCSV = useCallback(() => {
    ...
    aEl.href = url;
    aEl.download = `resultados-salvos-${new Date()
      .toISOString()
      .slice(0, 19)}.csv`;
  ...
  return (
    <div className="p-4 md:p-8 gap-8 flex flex-col h-full ">
      <Helmet>
        <title>Buscar Catálogo | Sistema Patrimônio</title>
        <meta
          name="description"
          content="Buscar Catálogo | Sistema Patrimônio"
        />
      </Helmet>
