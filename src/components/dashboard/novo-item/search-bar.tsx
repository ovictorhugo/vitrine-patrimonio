...
export function SearchBarNewItem() {
  ...
  // Novo array de termos a ser usado na navegação
  const nextItems = type === itemType
  ...
  // Chama a função final com os valores corretos
  handlePesquisaFinal(nextItems, type);
};

const handlePesquisaFinal = (items: { term: string; type: string }[], type: string) => {
  if (items.length === 0) {
  ...
  const terms = items.map(item => item.term).join(';').replace(/[()]/g, '');

  queryUrl.set('terms', terms);
  queryUrl.set('type_search', type);
