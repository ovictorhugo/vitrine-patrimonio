...
type Props = {
  notification: any; // aninhado ou achatado
  baseUrl: string;
  token: string;
...
function normalizeIconKey(name?: string) {
  ...
  const candidates = [
    cleaned,
    cleaned.toLowerCase(),
    cleaned.replace(/[-_](\w)/g, (_, c) => c.toUpperCase()),
...
function resolveTypeInfo(type: string | undefined, notificationsTypes: TypeInfo[]) {
...
export const NotificationItemAdmin = memo(function NotificationItemAdmin({
  ...
  // Pode não existir no GET /my
  const recipients: any[] = Array.isArray(notification?.recipients)
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-2xl mb-2 font-medium max-w-[450px] flex items-center gap-2">
              <IconComp size={24} className="shrink-0" />
              {detail?.title ?? "Notificação"}
            </DialogTitle>
            {detail?.description && (
              ...
              <div className="flex flex-col gap-1 p-2">
                {total === 0 ? (
                  ...
                  recipients.map((r: any) => {
                    ...
                    return (
                      <Button variant="ghost" key={r?.id} className="text-left justify-start h-auto">
          ...
          <DialogFooter>
            <div className="flex justify-between items-center w-full">
              <div className="flex text-sm gap-4 items-center text-gray-500">
                <div className="flex gap-2 items-center">
                  <Users size={12} className="text-gray-400" />
                  <span className="font-medium">{total}</span>
                  <span>destinatário{total === 1 ? "" : "s"}</span>
