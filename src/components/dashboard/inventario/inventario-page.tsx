...
async function getInventoryDetail(
  ...
  const available =
    typeof js.available === "boolean" ? js.available : !!js.avaliable;
  return {
    id: js.id,
    key: js.key,
    available,
...
export function InventarioPage() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const res = await fetch(
        `${urlGeral}statistics/catalog/count-by-workflow-status`,
        {
          method: "GET",
          headers: authHeaders,
        }
  ...
  const countReview =
    (statsMap["REVIEW_REQUESTED_VITRINE"] || 0) +
    (statsMap["REVIEW_REQUESTED_DESFAZIMENTO"] || 0);
  ...
  const confirmEnabled =
    !!currentInventory && deleteText.trim() === (currentInventory.key || "");
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      ...
      toast("Inventário excluído", {
        description: `Inventário ${currentInventory.key} removido com sucesso.`,
      });
      setDeleteOpen(false);
      setDeleteText("");
      setCurrentInventory(null);
  ...
  const handleClickRoom = useCallback(
    async (room: LocationMy) => {
      ...
      try {
        const assets = await getInventoryAssetsByLocation(
          urlGeral,
          currentInventory.id,
          room.id,
          token
        ...
        toast("Sala carregada", {
          description: `${room.location_name}: ${assets.length} item(ns).`,
        });
      } catch (e: any) {
  ...
  return (
    <div className="flex flex-col h-full">
      <Helmet>
        ...
        <meta
          name="description"
          content={`${currentInventory.key} | Sistema Patrimônio`}
        />
      </Helmet>
          ...
          <div className="justify-center  px-4 md:px-8 w-full mx-auto flex max-w-[1200px] flex-col items-center gap-2 py-8 md:py-12 md:pb-8 lg:py-24 lg:pb-20">
            ...
            <div className="mt-2 flex flex-wrap justify-center  gap-3 text-sm text-gray-500 items-center">
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`w-4 h-4 rounded-md ${
                    currentInventory?.available ? "bg-green-500" : "bg-red-500"
                  }`}
                />
                {currentInventory?.available ? "Disponível" : "Encerrado"}
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {formatDateTimeBR(currentInventory?.created_at)}
              </div>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          ...
          <div className="space-y-2 mb-4">
            ...
            {!!currentInventory &&
              deleteText.trim() &&
              deleteText.trim() !== currentInventory.key && (
