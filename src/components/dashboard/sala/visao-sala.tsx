...
export function VisaoSala() {
  ...
  useEffect(() => {
    setUrlBackground(`${urlGeral}location/upload/${room?.id}/cover`);
    setUrlPerfil(`${urlGeral}location/upload/${room?.id}/icon`);
  }, [room]);
  ...
  const handleUpload = (folder: UploadFolder, programId: string) => {
    ...
    input.onchange = async (event) => {
      ...
      try {
        ...
        if (folder === "profile") {
          setUrlPerfil(`${urlGeral}user/upload/${room?.id}/icon`);
        } else {
          setUrlBackground(`${urlGeral}user/upload/${room?.id}/cover`);
        }
  ...
  if (
    ...
    !(
      room &&
      room?.legal_guardian_id == user?.system_identity?.legal_guardian.id
    )
  ) {
    return (
      <div className="h-full bg-cover bg-center flex flex-col items-center justify-center bg-neutral-50 dark:bg-neutral-900">
        <div className="w-full flex flex-col items-center justify-center">
          ...
          <h1 className="text-center text-2xl md:text-4xl text-neutral-400 font-medium leading-tight tracking-tighter lg:leading-[1.1] ">
            Você não tem permissão para <br /> acessar as informações desta
            sala.
          </h1>
  ...
  return (
    <main className=" w-full grid grid-cols-1 ">
      <Helmet>
        ...
        <meta
          name="description"
          content={`${room.location_name} | Sistema Patrimônio`}
        />
                ...
                >
                  ...
                  >
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleUpload("background", room?.id || "")}
                      className="h-8 text-eng-blue hover:text-eng-blue"
                    >
                  ...
                  <p className="max-w-[750px] text-lg font-light text-foreground">
                    <div className="flex flex-wrap gap-4 ">
                      <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
                        <User size={12} />
                        {room.legal_guardian.legal_guardians_name}
                      </div>
                    </div>
                  </p>
                </div>
