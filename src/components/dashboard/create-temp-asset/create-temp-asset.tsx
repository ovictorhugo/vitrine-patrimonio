...
import { ChevronLeft, ArrowLeft, ArrowRight, LoaderCircle, Check } from "lucide-react";
...
const buildAssetsPayload = (form: PatrimonioSP, tl?: WizardState["trocar-local"]) => ({
...
const buildEtiquetaData = (form?: PatrimonioSP, tl?: WizardState["trocar-local"]) => {
...
export function CreateTempAsset() {
  ...
  useEffect(() => {
    setValid((prev) => {
      ...
      for (const s of STEPS) {
        if (next[s.key] === undefined) next[s.key] = s.key === "formulario-sp" ? false : undefined;
      }
      return next;
    });
  }, []);
  ...
  const onStateChangeFormularioSP = useCallback((st: PatrimonioSP) => {
    setWizardIfChanged(prev => ({ ...prev, "formulario-sp": st }));
  }, [setWizardIfChanged]);

  const onStateChangeTrocarLocal = useCallback((st: any) => {
    setWizardIfChanged(prev => ({ ...prev, "trocar-local": { ...(prev["trocar-local"] || {}), ...st } }));
  }, [setWizardIfChanged]);
  ...
  useEffect(() => {
    ...
    timeouts.push(setTimeout(() => setLoadingMessage("Gerando o patrimônio temporário..."), 4000));
    timeouts.push(setTimeout(() => setLoadingMessage("Estamos quase lá, continue aguardando..."), 8000));
    timeouts.push(setTimeout(() => setLoadingMessage("Só mais um pouco..."), 12000));
    timeouts.push(setTimeout(() => setLoadingMessage("Está demorando mais que o normal... finalizando o envio."), 16000));
  ...
  const handleFinish = useCallback(async () => {
    try {
      ...
      if (!formSP) {
        toast("Dados incompletos", { description: "Preencha o formulário antes de finalizar." });
        return;
      }
      ...
      setWizardIfChanged(prev => ({
        ...prev,
        "formulario-sp": { ...(prev["formulario-sp"] as PatrimonioSP), id: assetId },
  ...
  // === Render página ===
  const etiquetaData = buildEtiquetaData(wizard["formulario-sp"], wizard["trocar-local"]);
      ...
      <main className="flex flex-1 h-full lg:flex-row flex-col-reverse gap-8">
        <div className="w-full flex flex-col gap-8">
          <div className="flex gap-2">
            ...
            <h1 className="text-xl font-semibold tracking-tight">Criar patrimônio temporário</h1>
          </div>
                ...
                <TabsContent key={s.key} value={s.key} className="m-0 h-full">
                  {s.key === "formulario-sp" && (
                    <FormularioSpStep
                      ...
                      onValidityChange={onValidityChangeFactory("formulario-sp")}
                  ...
                  {s.key === "trocar-local" && (
                    <TrocarLocalStep
                      ...
                      flowShort={"desfazimento" /* mantemos aberto por padrão */}
              ...
              <div className="flex justify-between items-center h-fit">
                ...
                <div className="flex items-center">
                  ...
                  <Button
                    ...
                    disabled={isPenultimate ? !(valid["formulario-sp"] && valid["trocar-local"]) : !canGoNext}
                  >
