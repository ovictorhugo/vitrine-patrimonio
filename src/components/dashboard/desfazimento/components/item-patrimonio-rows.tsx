...
export function ItemPatrimonioRows(props: Props) {
  ...
  const imageUrls = (props.images ?? []).map((i) => buildImgUrl(i.file_path)).filter(Boolean) as string[];
  ...
  return (
    <div className={`flex group cursor-pointer rounded-lg ${selectedCls}`} onClick={handleRowClick}>
      {/* Barra colorida */}
      <div
