...
export function Roles({
  ...
  // editar role (⚠️ FIX: declarar corretamente)
  ...
  const [openUserDialogForRoleId, setOpenUserDialogForRoleId] = useState<
    string | null
  >(null);
  ...
  const [openPermissionsForRoleId, setOpenPermissionsForRoleId] = useState<
    string | null
  >(null);
  ...
  const [loadingRoleUsers, setLoadingRoleUsers] = useState<
  ...
  const fetchRoles = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${urlGeral}roles/?offset=${encodeURIComponent(
          offset
  ...
  const fetchRoleUsers = useCallback(
    async (roleId: string) => {
      try {
        ...
        setRoleUsers((prev) => ({
          ...prev,
          [roleId]: Array.isArray(data?.users) ? data!.users : [],
        }));
      } catch (e: any) {
        toast("Erro ao carregar membros do cargo", {
          description: e?.message || String(e),
          action: { label: "Fechar", onClick: () => {} },
        });
  ...
  const isLastPage = roles.length < limit;
    ...
    try {
      if (!name.trim()) {
        toast("Informe o nome do cargo", {
          description: "O campo 'Nome' está vazio.",
          action: { label: "Fechar", onClick: () => {} },
        });
        return;
      }
      ...
      // volta pra primeira página e refaz a busca
      setOffset(0);
      await fetchRoles();

      toast("Cargo criado com sucesso!", {
        description: `“${name.trim()}” foi adicionado.`,
        action: { label: "Fechar", onClick: () => {} },
      });
      setName("");
      setDescription("");
  ...
  const handleEditConfirm = async () => {
    ...
    try {
      ...
      toast("Cargo atualizado", {
        description: `“${editName.trim()}” foi salvo.`,
        action: { label: "Fechar", onClick: () => {} },
      });
      setEditOpen(false);
      setEditTarget(null);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setDeleting(true);
      const res = await fetch(`${urlGeral}roles/${deleteTarget.id}`, {
        method: "DELETE",
        headers: authHeaders,
      });
  ...
  const handleAddUserToRole = async (roleId: string) => {
    ...
    try {
      const res = await fetch(
        `${urlGeral}roles/${roleId}/users/${selectedUser.id}`,
      ...
      toast("Usuário adicionado", {
        description: `${selectedUser.username} foi adicionado ao cargo.`,
        action: { label: "Fechar", onClick: () => {} },
      });
      setOpenUserDialogForRoleId(null);
      setSelectedUser(null);
    } catch (e: any) {
  ...
  const handleRemoveUserFromRole = async (roleId: string, userId: string) => {
    try {
      ...
      toast("Usuário removido", {
        description: `O usuário foi removido do cargo.`,
        action: { label: "Fechar", onClick: () => {} },
      });
    } catch (e: any) {
  ...
  const handleOpenPermissionsForRole = async (
    ...
    if (!open) setPermPopoverOpen(false);
  };

  const handleLinkPermissionToRole = async (roleId: string) => {
    if (!selectedPermissionId.trim()) {
      toast("Selecione uma permissão", {
        description: "Escolha uma permissão no seletor para vincular.",
        action: { label: "Fechar", onClick: () => {} },
      });
      return;
    }
    try {
      const res = await fetch(
        `${urlGeral}roles/${roleId}/permissions?permission_id=${selectedPermissionId}`,
        { method: "POST", headers: authHeaders }
      );
      ...
      toast("Permissão vinculada", {
        description: `Permissão associada ao cargo.`,
        action: { label: "Fechar", onClick: () => {} },
      });
      setSelectedPermissionId("");
    } catch (e: any) {
      toast("Erro ao vincular permissão", {
        description: e?.message || String(e),
        action: { label: "Fechar", onClick: () => {} },
      });
    }
  ...
  const handleRemovePermissionFromRole = async (
    ...
    try {
      ...
      const res = await fetch(
        `${urlGeral}roles/${roleId}/permissions/${permissionId}`,
      ...
      toast("Permissão removida", {
        description: `Permissão dissociada do cargo.`,
        action: { label: "Fechar", onClick: () => {} },
      });
    } catch (e: any) {
      toast("Erro ao remover permissão", {
        description: e?.message || String(e),
        action: { label: "Fechar", onClick: () => {} },
      });
    } finally {
  ...
  const handleCreateGlobalPermission = async () => {
    try {
      if (!newPermName.trim()) {
        toast("Preencha o nome da permissão", {
      ...
      toast("Permissão criada", {
        description: `“${newPermName.trim()}” (${generatedCode}) foi criada no catálogo global.`,
        action: { label: "Fechar", onClick: () => {} },
      });

      setNewPermName("");
      setNewPermDesc("");
      setOpenCreatePerm(false);
      setPermPopoverOpen(false);
    } catch (e: any) {
      toast("Erro ao criar permissão", {
        description: e?.message || String(e),
        action: { label: "Fechar", onClick: () => {} },
      });
    } finally {
      setCreatingGlobalPerm(false);
    }
  };
      ...
      <Dialog open={openCreatePerm} onOpenChange={setOpenCreatePerm}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleCreateGlobalPermission}
              disabled={creatingGlobalPerm}
            >
              {creatingGlobalPerm ? (
                <Loader2 className="animate-spin " size={16} />
              ) : (
                                      ...
                                      <div className="grid gap-2">
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                                          <Popover
                                            ...
                                            onOpenChange={setPermPopoverOpen}
                                          >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription className="text-zinc-500 ">
              ...
              Para confirmar, digite exatamente o nome do cargo:{" "}
            ...
            {!!deleteTarget &&
              ...
              deleteText.trim() !== deleteTarget.name && (
                <p className="text-xs text-red-500">
                  O texto digitado não corresponde ao nome do cargo.
                </p>
              )}
          </div>
